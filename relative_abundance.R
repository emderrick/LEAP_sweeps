...
EPSPS <- read_csv("refined data files/EPSPS_MAG_Classification.csv")
EPSPS <- EPSPS[, c("mag", "Classification")]
args <- read_csv("refined data files/arg_hits.csv")
rel_abun <- read_tsv("refined data files/T1_refined_coverM.tsv")
...
write.csv(rel_abun, "refined data files/MAG_rel_abun.csv", row.names = F)
...
write.csv(rel_abun_change, "refined data files/MAG_rel_abun_change.csv", row.names = F)
...
write.csv(rel_abun_AVG, "refined data files/MAG_avg_abun.csv", row.names = F)
