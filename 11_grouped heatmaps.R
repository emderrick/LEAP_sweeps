...
all_samples <- read_csv("refined data files/chapter_1_sample_names.csv")
...
for(i in 1:length(mag_list)){
  ...
  bin_snv$Name <- factor(bin_snv$Name, levels = c("CTRL A", "CTRL B", "CTRL C", "CTRL D", "CTRL E", "GBH A", "GBH B", "GBH C", "GBH D"))
  ...
  bin_snv_sum <- ggplot(bin_sum, aes(x = Name, y = SNVs_Mbp, fill = Treatment))+
    ...
    theme(text = element_text(size = 8), axis.text = element_text(colour = "black"), strip.text.x = element_text(size = 12), legend.position = "none")+
  ...
  bin_sns_sum <- ggplot(bin_sum, aes(x = Name, y = SNSs_Mbp, fill = Treatment))+
    ...
    theme(text = element_text(size = 8), axis.text = element_text(colour = "black"), strip.text.x = element_blank())+
    facet_wrap(~Time, scales = "fixed")
  ...
  bin_snvs <- ggplot(bin_snv, aes(x = Name, y = final_ref_freq, colour = Treatment)) +
    ...
    theme(text = element_text(size = 8), axis.text = element_text(colour = "black"), strip.text.x = element_blank(), legend.position = "none")+
    facet_wrap(~Time, scales = "fixed")
  
  bin_plot <- bin_snv_sum / bin_sns_sum / bin_snvs
  ggsave(paste("refined figures/", mag_list[i], "_snvs.pdf", sep = ""), bin_plot, limitsize = F, width = 12, height = 8)
}
