...
mag_cov <- filter(all_mags, mag %in% mag_list)
mag_cov <- mag_cov %>% filter(!(mag=="L3_MAG_00058" & new_time=="1"))
mag_cov <- mag_cov %>% filter(!(mag=="L3_MAG_00058" & new_time=="3"))
mag_cov <- mag_cov %>% filter(!(mag=="L4_MAG_00099" & new_time=="3"))
mag_cov <- mag_cov %>% filter(!(mag=="L8_MAG_00011" & new_time=="3"))
mag_cov <- mag_cov %>% filter(!(mag=="L7_MAG_00043" & new_time=="3"))
mag_cov <- mag_cov %>% filter(!(mag=="L7_MAG_00028" & new_time=="3"))
mag_cov <- mag_cov %>% filter(!(mag=="I4_MAG_00065" & new_time=="1"))
mag_cov <- mag_cov %>% filter(!(mag=="I4_MAG_00065" & new_time=="3"))
mag_cov <- mag_cov %>% filter(!(mag=="L8_MAG_00042" & new_time=="3"))
mag_cov <- mag_cov %>% filter(!(mag=="L7_MAG_00020" & new_time=="1" & pond=="L7"))
mag_cov <- mag_cov %>% filter(!(mag=="L2_MAG_00052" & new_time=="1"))
...
mags <- mags %>% group_by(full_group) %>% mutate(median_cov = median(position_coverage))
mags <- mags %>% filter(!(position_coverage > median_cov*3)) 
mags <- mags %>% filter(!(position_coverage < median_cov/3))
