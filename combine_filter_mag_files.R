...
sample_names <- read_csv("subsamp data files/chapter_1_sample_names.csv")

genome_files <- list.files("subsamp data files/T1_subsamp_inStrain/", recursive = T, pattern = ".*genome_info.tsv", full.names = T)

all_mags <- data.frame()
for(i in 1:length(genome_files)){
  pond_time_mags <- read.table(genome_files[i], sep="\t", header = T)
  pond_time_mags$Sample <- genome_files[i] %>% substr(41,52)
...
write.csv(mapping_info, "subsamp data files/mag_subsamp_mapping_info.csv", row.names = F)

scaffold_files <- list.files("subsamp data files/T1_subsamp_inStrain/", recursive = T, pattern = ".*scaffold_info.tsv", full.names = T)

all_scaffolds <- data.frame()
for(i in 1:length(scaffold_files)){
  pond_time_scaffolds <- read.table(scaffold_files[i], sep = "\t", header = T)
  pond_time_scaffolds$Sample <- scaffold_files[i] %>% substr(41,52)
  all_scaffolds <- rbind(all_scaffolds, pond_time_scaffolds)
} 

mag_info <- all_mags[, c(1:3,5,8,31,33,34)]

mag_scaffold_info <- read_tsv("subsamp data files/T1_refined.stb", col_names = F)
...
write.csv(mag_info, "subsamp data files/T1_subsamp_mag_info.csv", row.names = F)

SNV_files <- list.files("subsamp data files/T1_subsamp_inStrain/", recursive = T, pattern = ".*SNVs.tsv", full.names = T)

for(i in 1:length(SNV_files)){
  pond_time_SNV <- read.table(SNV_files[i], sep = "\t", header = T)
  Sample <- SNV_files[i] %>% substr(41,52)
  ...
  write.csv(SNV_mag, paste("subsamp data files/T1_subsamp_filt_SNVs_", Sample, ".csv", sep = ""), row.names = F)
}  
