...
all_sum$treatment <- str_sub(all_sum$name, end=-8)
all_sum$graph_name <- with(all_sum, ifelse(mag=="L7_MAG_00020", name, str_sub(all_sum$name, end = -6)))
...
all_sum_long$graph_name <- with(all_sum_long, ifelse(mag=="L7_MAG_00020", name, str_sub(all_sum_long$name, end = -6)))
...
select_snv_heat <- ggplot(select_snv, aes(x = graph_name, y = reorder(groups, all_mean), fill= final_ref_freq)) +
  ...
  theme(axis.text.y=element_blank(), axis.ticks.y=element_blank(), axis.title.y=element_blank(), axis.title.x = element_blank(), axis.ticks.x=element_blank(), 
        plot.title = element_text(hjust = 0.5, size=20), text=element_text(size=15), panel.spacing=unit(1, "cm"))+
...
select_snv_sum <- ggplot(subset(select_sum_long, class=="SNVs"), aes(x = graph_name, y=((divergent_sites/mag_length)*10^6), fill=treatment))+
  ...
  theme_classic()+
  scale_fill_manual(values=c("#A5D6A7","#CC9999"))+
  theme(legend.title=element_blank(), axis.title.x = element_blank(), plot.title = element_blank(), text=element_text(size=15), panel.spacing=unit(0.05, "cm"))+
  labs(y= "SNVs / Mbp")+
...
select_snv_frac <- ggplot(select_sum, aes(x = graph_name, y=SNSs/(SNSs+SNVs)))+
  ...
  theme_classic()+
  theme(legend.title=element_blank(), axis.title.x = element_blank(), plot.title = element_blank(), text=element_text(size=15), panel.spacing=unit(0.4, "cm"))+
  ylim(0, 1)+
...
other_snv_sum <- ggplot(subset(other_sum_long, class=="SNVs"), aes(x = graph_name, y=((divergent_sites/mag_length)*10^6), fill=treatment))+
  geom_bar(stat="identity")+ 
  theme_classic()+
  scale_fill_manual(values=c("#E0E0E0","#67032F"))+
...
other_all <- ggarrange(other_snv_heat, other_snv_sum, other_snv_frac, align="hv", ncol=1, nrow=3)
