...
all_snv$graph_name <- with(all_snv, ifelse(mag == "L7_MAG_00020",  str_sub(new_name, -7), str_sub(new_name, -7, -7)))
all_snv$graph_name <- sub('at ', '', all_snv$graph_name)
...
all_sum$graph_name <- with(all_sum, ifelse(mag == "L7_MAG_00020",  str_sub(new_name, -7), str_sub(new_name, -7, -7)))
all_sum$graph_name <- sub('at ', '', all_sum$graph_name)
...
select_mag_labs <- (c(I4_MAG_00006 = "SJAQ100 sp016735685", I4_MAG_00065 = "Roseomonas sp.", L3_MAG_00058 = "Prosthecobacter sp.",
                      L7_MAG_00020 = "Sphingorhabdus_B sp. ", L8_MAG_00011 = "UBA953 sp.", L8_MAG_00019 = "UA16 family "))
...
select_snv_heat <- ggplot(select_snv, aes(x = graph_name, y = reorder(groups, all_mean), fill = final_ref_freq)) +
  ...
  theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(), axis.title.x = element_blank(), text = element_text(size = 30), axis.title.y = element_text(face = "bold"),
        strip.text.x.top = element_text(size = 35, face = "bold"), panel.spacing = unit(1, "cm"), axis.text.x = element_text(colour = "black", vjust = -1), legend.title = element_text(face = "bold"))+
...
select_snv_sum <- ggplot(subset(select_sum_long, class == "SNVs"), aes(x = graph_name, y=((divergent_sites/mag_length)*10^6), fill = treatment))+
  ...
  theme(axis.title.x = element_blank(), strip.text.x = element_blank(), axis.text = element_text(color = "black"), axis.title.y = element_text(face = "bold"),
        text = element_text(size = 30), panel.spacing = unit(0.05, "cm"), legend.title = element_text(face = "bold"))+
...
select_snv_frac <- ggplot(select_sns_sum, aes(x = graph_name, y=SNSs/total))+
