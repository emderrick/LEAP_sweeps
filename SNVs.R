...
small_MAG_SNVs_hor<- small_MAG_SNVs_hor %>% rowwise %>% mutate(all_mean = mean(c_across(contains("at")), na.rm=T))
small_MAG_SNVs_hor<- small_MAG_SNVs_hor %>% rowwise %>% mutate(control_mean = mean(c_across(starts_with("Control")), na.rm=T))
small_MAG_SNVs_hor<- small_MAG_SNVs_hor %>% rowwise %>% mutate(GBH_mean = mean(c_across(starts_with("GBH")), na.rm=T))
...
class_MAG_SNVs<-all_MAG_SNVs %>% select(c('groups','name','class','mag_length'))
...
L3_MAG_00058_SNVs <- subset(small_MAG_SNVs_hor, mag=="L3_MAG_00058") %>% 
  select(c("groups", "mag", "gene", "Control C at T2", "Control D at T2","GBH C at T2","GBH D at T2", "all_mean", "control_mean", "GBH_mean", "abs_val")) %>%
...
L4_MAG_00099_SNVs <- subset(small_MAG_SNVs_hor, mag=="L4_MAG_00099") %>% 
  select(c("groups", "mag", "gene", "Control D at T2", "GBH A at T2", "GBH B at T2","GBH C at T2","GBH D at T2", "all_mean", "control_mean", "GBH_mean", "abs_val")) %>%
  pivot_longer(cols=contains("at"),  names_to="name", values_to="final_ref_freq", values_drop_na=F) %>% 
  left_join(class_MAG_SNVs, by=c("groups", "name"))
...
L8_MAG_00019_SNVs <- subset(small_MAG_SNVs_hor, mag=="L8_MAG_00019") %>% 
  select(c("groups", "mag", "gene", "Control E at T2", "GBH A at T2","GBH B at T2","GBH D at T2", "all_mean", "control_mean", "GBH_mean", "abs_val")) %>%
  pivot_longer(cols=contains("at"),  names_to="name", values_to="final_ref_freq", values_drop_na=F) %>% 
  left_join(class_MAG_SNVs, by=c("groups", "name"))
...
L8_MAG_00011_SNVs <- subset(small_MAG_SNVs_hor, mag=="L8_MAG_00011") %>% 
  select(c("groups", "mag", "gene", "Control E at T2", "GBH A at T2","GBH D at T2", "all_mean", "control_mean", "GBH_mean", "abs_val")) %>%
  pivot_longer(cols=contains("at"),  names_to="name", values_to="final_ref_freq", values_drop_na=F) %>% 
  left_join(class_MAG_SNVs, by=c("groups", "name")) 
...
L7_MAG_00043_SNVs <- subset(small_MAG_SNVs_hor, mag=="L7_MAG_00043") %>% 
  select(c("groups", "mag", "gene", "Control D at T2", "GBH B at T2","GBH C at T2", "all_mean", "control_mean", "GBH_mean", "abs_val")) %>%
  pivot_longer(cols=contains("at"),  names_to="name", values_to="final_ref_freq", values_drop_na=F) %>% 
  left_join(class_MAG_SNVs, by=c("groups", "name"))
...
L7_MAG_00028_SNVs <- subset(small_MAG_SNVs_hor, mag=="L7_MAG_00028") %>% 
  select(c("groups", "mag", "gene", "Control E at T2", "GBH A at T2","GBH B at T2","GBH C at T2", "all_mean", "control_mean", "GBH_mean", "abs_val")) %>%
  pivot_longer(cols=contains("at"),  names_to="name", values_to="final_ref_freq", values_drop_na=F) %>% 
  left_join(class_MAG_SNVs, by=c("groups", "name"))
...
I4_MAG_00006_SNVs <- subset(small_MAG_SNVs_hor, mag=="I4_MAG_00006") %>% 
  select(c("groups", "mag", "gene", "Control B at T2", "Control E at T2","GBH A at T2","GBH D at T2", "all_mean", "control_mean", "GBH_mean", "abs_val")) %>%
  pivot_longer(cols=contains("at"),  names_to="name", values_to="final_ref_freq", values_drop_na=F) %>% 
  left_join(class_MAG_SNVs, by=c("groups", "name"))
...
I4_MAG_00065_SNVs <- subset(small_MAG_SNVs_hor, mag=="I4_MAG_00065") %>% 
  select(c("groups", "mag", "gene", "Control A at T2","Control B at T2","Control C at T2", "Control D at T2","Control E at T2","GBH B at T2", "all_mean", "control_mean", "GBH_mean", "abs_val")) %>%
  pivot_longer(cols=contains("at"),  names_to="name", values_to="final_ref_freq", values_drop_na=F) %>% 
  left_join(class_MAG_SNVs, by=c("groups", "name"))
...
L7_MAG_00020_SNVs <- subset(small_MAG_SNVs_hor, mag=="L7_MAG_00020") %>% 
  select(c("groups", "mag", "gene", "Control A at T1","Control A at T2","Control C at T1", "Control C at T2","Control D at T1",
...
L8_MAG_00042_SNVs <- subset(small_MAG_SNVs_hor, mag=="L8_MAG_00042") %>% 
  select(c("groups", "mag", "gene", "Control A at T2", "Control C at T2","Control D at T2","Control E at T2","GBH D at T2", "all_mean", "control_mean", "GBH_mean", "abs_val")) %>%
  pivot_longer(cols=contains("at"),  names_to="name", values_to="final_ref_freq", values_drop_na=F) %>% 
  left_join(class_MAG_SNVs, by=c("groups", "name"))
...
L2_MAG_00052_SNVs <- subset(small_MAG_SNVs_hor, mag=="L2_MAG_00052") %>% 
  select(c("groups", "mag", "gene", "Control A at T2", "Control B at T2","Control D at T2","Control E at T2","GBH A at T2", "all_mean", "control_mean", "GBH_mean", "abs_val")) %>%
  pivot_longer(cols=contains("at"),  names_to="name", values_to="final_ref_freq", values_drop_na=F) %>% 
  left_join(class_MAG_SNVs, by=c("groups", "name"))
