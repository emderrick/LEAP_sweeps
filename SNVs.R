...
small_MAG_SNVs<- all_MAG_SNVs %>% select(c('scaffold', 'position', 'length', 'groups', 'gene', 'mag', 'mag_length', 'new_name', 'final_ref_freq'))
...
test_hor <- spread(test, key=new_name, value=final_ref_freq)
test_hor$all_mean <-rowMeans(test_hor[c(8:20)], na.rm=T)
test_hor$control_mean <-rowMeans(test_hor[c(9,10,12,14,15)], na.rm=T)
test_hor$GBH_mean <-rowMeans(test_hor[c(17,18,19,20)], na.rm=T)
test_hor$abs_val<- abs(test_hor$control_mean - test_hor$GBH_mean)
...
for(MAG in mag_list){
  MAG_SNVs <- subset(small_MAG_SNVs_hor, mag==MAG) %>% 
  MAG_index<-which(names(ponds_list)==MAG)
  select(c(1:7, ponds_list[MAG_index], 21:24)) %>%
