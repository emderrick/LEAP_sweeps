...
SNV_files <- list.files("refined data files/", recursive = T, pattern = "filt_SNVs", full.names = T)
...
write.csv(mag_SNVs, "refined data files/good_refined_MAG_SNVs.csv", row.names = F)

mag_scaffolds <- as.data.frame(mag_SNVs$scaffold)
mag_scaffolds <- unique(mag_scaffolds)
colnames(mag_scaffolds) <- "scaffold"
write.csv(mag_scaffolds, "refined data files/good_refined_mag_scaffolds.csv", row.names = F)
