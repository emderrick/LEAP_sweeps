...
snv_freq$OR_sig <- with(snv_freq, ifelse(OR < 1 & fdr < 0.05, "significantly depleted", "p > 0.05"))
...
gbh_loss$OR_sig <- with(gbh_loss, ifelse(OR < 1 & fdr < 0.05, "significantly depleted", "p > 0.05"))
...
gbh_loss_plot <- ggplot(gbh_loss, aes(x = category, y = -log10(p), fill = OR_sig))+
  ...
  theme(title = element_text(size = 10),
        ...
        legend.title = element_text(size = 12, colour = "black"))+
  ylim(0, 10)+
  labs(x = "", y = "-log10(p-value)", fill = "", title = "GBH gene loss")

cog_plot <- ggarrange(snv_plot, gbh_loss_plot, common.legend = TRUE, legend = "top", nrow = 2, labels = c("A", "B"))
