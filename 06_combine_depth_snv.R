...
depth_files <- list.files("data files/inStrain depth files", recursive = T, pattern = "depth.csv", full.names = T)
...
for(i in 1:length(depth_files)){
  ...
  sample_depth$Sample <- depth_files[i] %>% substr(33,44)
