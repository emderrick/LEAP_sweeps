...
pond_vs_SNV_boxplot <- ggplot(mag_SNVs, aes(x = Name, y = SNVs_Kbp, fill = Time, colour = Time))+
  geom_boxplot(outliers = F)+
...
pond_vs_SNS_boxplot <- ggplot(mag_SNVs, aes(x = Name, y = SNSs_Kbp, fill = Time, colour = Time))+
  geom_boxplot(outliers = F)+
  geom_point(aes(colour = Time), size = 0.25, position = position_jitterdodge(jitter.width = 0.25))+
  scale_fill_manual(values = c("white", "white"))+
  scale_colour_manual(values = c("darkgreen", "darkmagenta"))+
  labs(x = "", y = "Fixed substiturions per Kbp")+  
  theme_classic()+
  theme(axis.text = element_text(size = 10, colour = "black"),
        axis.title = element_text(size = 12, colour = "black"),
        legend.text = element_text(size = 12, colour = "black"),
