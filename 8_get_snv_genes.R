...
snv_frequency <- read_csv("data files/all_snv_frequency.csv")
...
sig_changes <- subset(snv_frequency, GBH_CTRL_change_abs >= 0.5 & GBH_change_abs >= 0.5)
...
write.csv(significant_nonsyn_genes, "data files/nonsyn_allele_shifts_significant_genes.csv", row.names = F)

significant_syn_genes <- left_join(sig_syn_sum, background_cog) 
write.csv(significant_syn_genes, "data files/syn_allele_shifts_significant_genes.csv", row.names = F)
